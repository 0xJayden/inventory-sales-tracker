...
pub async fn get_clients() -> Result<Vec<Client>, Errorr> {
    ...
    let clients = sqlx::query_as!(Client,
                                "SELECT * FROM Client"
                               )
        .fetch_all(&pool)
        .await?;
...
pub async fn get_client(i: i64) -> Result<Client, Errorr> {
    ...
    let client = sqlx::query_as!(Client,
...
impl ClientState {
        ...
        if self.edit_client {
            Some(
                ...
                .push(
                    ...
                    .push(
                        text_input_column("Name", &self.client_to_edit.name, |input| {
                            AppMessage::Client(
                                ClientMessage::NameInput(input, true),
                                )
                        })
                        )
                    .push(
                        text_input_column("Address", &self.client_to_edit.address, |input| {
                            AppMessage::Client(
                                ClientMessage::AddressInput(input, true),
                                )
                        })
                        )
                    .push(
                        text_input_column("Email", &self.client_to_edit.email.clone().unwrap_or("".to_string()), |input| {
                            AppMessage::Client(
                                ClientMessage::EmailInput(input, true),
                                )
                        })
                        )
                    .push(
    ...
    pub fn view(&self) -> Element<AppMessage> {
        layout(
            ...
            .push(Text::new("Clients".to_string())
                  .size(24))
            .push(Row::new()
                  .push(
                    add_button("Add Cient", AppMessage::Client(ClientMessage::ShowAddClient))
                    )
                  .padding(12))
                  .push_maybe(self.create_view())
                  .push_maybe(self.edit_view())
            .push(Container::new(
                    table_header(&["Name", "Address", "Email"])
                    .push(Scrollable::new(
                            Column::new()
                            .extend(self.clients.iter().map(
                                    |client| 
                                    Button::new(
                                        Container::new(
                                            Row::new()
                                            ...
                                            .push(table_column(&client.email.clone().unwrap_or("".to_string()))))
                ...
                .push(
                    ...
                    .push(
                        text_input_column(
                            ...
                            |input| { 
                                AppMessage::Client(ClientMessage::NameInput(input, false))
                            }
                            )
                        )
                    .push(
                    ...
                    .push(
                        text_input_column(
                            ...
                            |input| {
                                AppMessage::Client(ClientMessage::EmailInput(input, false))
                            })
                        )
                    .push(
                        Row::new()
                        .push(
                            Button::new("Submit")
                            .on_press(AppMessage::Client(
                                    ClientMessage::Submit(false),
                                    ))
                            .style(CustomMainButtonStyle)
                            ),
                            )
