...
use crate::{
    components::{add_button, layout, table_column, table_header, table_row_qty_style, table_style, text_input_column, CustomButtonStyle, CustomMainButtonStyle},
    error::Errorr,
    AppMessage
};
    ...
    pub fn view(&self) -> Element<AppMessage> {
        layout(
            ...
            .push(
                Row::new()
                .push(
                    add_button("Add Part", AppMessage::Parts(PartsMessage::ShowAddPart))
                    )
                .padding(12),
                )
            ...
            .push(
                Container::new(
                    table_header(&["Name", "Cost", "Units Left", "Total Spent", "Total Purchased"])
                    .push(
                        Scrollable::new(
                            Column::new()
                            .extend(
                                ...
                                .map(|item| {
                                    Button::new(
                                        Container::new(
                                            ...
                                            .push(table_column(
                                                    &item.units_left.to_string(),
                                                    ))
                                            .push(table_column(
                                                    &format!("${:.2}", item.total_spent),
                                                    ))
                                            .push(table_column(
